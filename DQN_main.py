...
for episode in range(n_episodes):
    ...
    obs_arr = [np.array(mpimg.imread('img_qbert_4.png'))]
    for obs in obs_arr:
        
        filtered_obs = filter_obs(obs)
        prev_frames[0] = filter_obs(np.array(mpimg.imread('img_qbert_1.png')))
        prev_frames[1] = filter_obs(np.array(mpimg.imread('img_qbert_2.png')))
        prev_frames[2] = filter_obs(np.array(mpimg.imread('img_qbert_3.png')))
    ...
    if episode % print_freq == 0:
        ...
        np.save('Rewards_DQN_Qbert.npy', all_rewards)
        np.save('Q_DQN_Qbert.npy', q_rewards)
...
for episode in range(n_episodes):
    ...
    if episode % print_freq == 0:
        ...
        np.save('Rewards_DQN_QbertExpReplay.npy', all_rewards)
        np.save('Q_DQN_QbertExpReplay.npy', q_rewards)